...
set ylabel 'Normalized PAPI_VEC_DP';
set xlabel 'Time (ms)';
set xtics nomirror format "%.02f";
set xtics ( 0.0 , 1./5.*X_LIMIT, 2./5.*X_LIMIT, 3./5.*X_LIMIT, 4./5.*X_LIMIT, 5./5.*X_LIMIT, X_LIMIT/FACTOR);
set xrange [0:X_LIMIT*1./FACTOR];
set y2label 'PAPI_VEC_DP rate (in Mevents/s)';

# Mean rate
set label "" at first X_LIMIT*1./FACTOR, second 1285.31 point pt 3 ps 2 lc rgbcolor "#707070";

# Breakpoints
# Unneeded phases separators, nb. breakpoints = 2

# Data accessors to CSV
sampleexcluded(ret,c,r,g,t) = (c eq 'PAPI_VEC_DP' && r eq 'Cluster_2' && g == 0  && t eq 'e') ? ret : NaN;
sampleunused(ret,c,r,g,t) = (c eq 'PAPI_VEC_DP' && r eq 'Cluster_2' && g ==  0 && t eq 'un') ? ret : NaN;
sampleused(ret,c,r,g,t) = (c eq 'PAPI_VEC_DP' && r eq 'Cluster_2' && g == 0 && t eq 'u') ? ret : NaN;
interpolation(ret,c,r,g) = (c eq 'PAPI_VEC_DP' && r eq 'Cluster_2' && g == 0 ) ? ret : NaN;
slope(ret,c,r,g) = (c eq 'PAPI_VEC_DP' && r eq 'Cluster_2' && g == 0 ) ? ret : NaN;

# Plot command
plot \
NaN ti 'Mean PAPI_VEC_DP rate' w points pt 3 lc rgbcolor "#707070" ,\
