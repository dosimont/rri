...
# Mean rate
set label "" at first X_LIMIT*1./FACTOR, second 597.65 point pt 3 ps 2 lc rgbcolor "#707070";

# Breakpoints
# Unneeded phases separators, nb. breakpoints = 2

# Data accessors to CSV
sampleexcluded(ret,c,r,g,t) = (c eq 'RESOURCE_STALLS:SB' && r eq 'Cluster_9' && g == 0  && t eq 'e') ? ret : NaN;
sampleunused(ret,c,r,g,t) = (c eq 'RESOURCE_STALLS:SB' && r eq 'Cluster_9' && g ==  0 && t eq 'un') ? ret : NaN;
sampleused(ret,c,r,g,t) = (c eq 'RESOURCE_STALLS:SB' && r eq 'Cluster_9' && g == 0 && t eq 'u') ? ret : NaN;
interpolation(ret,c,r,g) = (c eq 'RESOURCE_STALLS:SB' && r eq 'Cluster_9' && g == 0 ) ? ret : NaN;
slope(ret,c,r,g) = (c eq 'RESOURCE_STALLS:SB' && r eq 'Cluster_9' && g == 0 ) ? ret : NaN;

# Plot command
plot \
NaN ti 'Mean RESOURCE_STALLS:SB rate' w points pt 3 lc rgbcolor "#707070" ,\
'nemo.exe.128tasks.chop1.clustered.codeblocks.fused.any.any.any.dump.csv' u ($4*FACTOR):(sampleexcluded($6, strcol(5), strcol(2), $3, strcol(1))) ti 'Excluded samples (217)' axes x2y1 w points pt 3 lc rgbcolor '#A0A0A0',\
'nemo.exe.128tasks.chop1.clustered.codeblocks.fused.any.any.any.dump.csv' u ($4*FACTOR):(sampleused($6, strcol(5), strcol(2), $3, strcol(1))) ti 'Used samples (311)' axes x2y1 w points pt 3 lc rgbcolor '#FF0000',\
